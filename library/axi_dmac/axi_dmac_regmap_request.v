...
module axi_dmac_regmap_request #(
  ...
  localparam DMAC_DEF_SRC_ADDR_LOC = !HAS_AUTORUN ? 'h00 :
      DMAC_DEF_SRC_ADDR[DMA_AXI_ADDR_WIDTH-1:BYTES_PER_BEAT_WIDTH_SRC];
  localparam DMAC_DEF_DEST_ADDR_LOC = !HAS_AUTORUN ? 'h00 :
  ...
  if (ENABLE_FRAME_LOCK == 1) begin
    ...
    localparam DMAC_DEF_FLOCK_CFG_WAIT_WRITER = (HAS_AUTORUN == 0) ? 'h0 :
    ...
    reg                          up_dma_flock_wait_writer = DMAC_DEF_FLOCK_CFG_WAIT_WRITER;
    ...
    always @(posedge clk) begin
      if (reset == 1'b1) begin
        ...
        up_dma_flock_wait_writer <= DMAC_DEF_FLOCK_CFG_WAIT_WRITER;
